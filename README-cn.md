...
- **动画过渡** — 播放场景间的缓动过渡动画，并预览移动路径；每个场景可独立设置过渡时长
- **音乐同步** — 加载本地 MP3 文件；场景自动映射到音乐时间轴，进度条显示场景标记；播放时自动切换当前场景；点击或拖拽进度条可快速跳转
- **场景时间设置** — 每个场景可单独配置开始时间和过渡时长；加载音乐后自动将场景均匀分布到音乐时间轴
...
5. **添加更多场景** — 位置将从上一场景复制为起点，逐一重新排列站位
6. 点击音乐面板的 **♪ Load** 按钮加载本地 MP3 — 场景将自动均匀分布到音乐时间轴
7. 在右侧 Animation 面板微调每个场景的**开始时间（Start）**和**过渡时长（Duration）**
8. 在底部**场景备注栏**记录编排构思
9. 点击 **▶ Play** 预览过渡动画效果，或在音乐进度条上点击任意位置跳转
10. **↓ ZIP** 导出完整备份（含音乐和背景图）；**↓ JSON** 轻量备份；按需为每个场景**↓ PNG**

## 界面布局

```
┌────────────────────────────────────────────────────────────────┐
│  工具栏：场景 | 播放 | 缩放 | 网格 | 撤销/重做 | JSON/ZIP导出  │
├──────────────┬───────────────────────────┬─────────────────────┤
│  左侧面板    │                           │  右侧面板           │
│  演员列表    │        舞台画布            │  舞台尺寸设置       │
│  + 添加演员  │       （SVG 渲染）         │  背景图片           │
...
- SVG 渲染，基于世界坐标变换组
- HTML5 `<audio>` API 实现音乐播放与时间同步
- [JSZip](https://stuk.github.io/jszip/)（CDN）实现 ZIP 打包导出 / 导入
- 无框架、无构建工具、无后端
